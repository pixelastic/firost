 ...
 **/
export async function copy(userSource, userDestination, userOptions = {}) {
  const options = {
    cwd: callerDir(),
    ...userOptions,
  };
  ...
  let list;
  try {
    list = await getMap(userSource, userDestination, options);
  } catch (err) {
    const newCode = err.code.replace('COPY_OR_MOVE', 'COPY');
