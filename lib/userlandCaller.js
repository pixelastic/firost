...
export function userlandCaller() {
  ...
  // if (!result) {
  //   throw firostError(
  //     'ERROR_NO_USERLAND_CALLER',
  //     [
  //       'Could not find a caller outside of firost.',
  //       'This means you are calling a userland-only function from inside firost code or tests.',
  //       'Functions like packageRoot(), gitRoot(), and absolute() without arguments can only be called from userland.',
  //       "If you need to reference firost's own roots, use packageRoot(dirname()) or gitRoot(dirname()) instead.",
  //     ].join('\n'),
  //   );
  // }
  //
