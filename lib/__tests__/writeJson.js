...
describe('writeJson', () => {
  ...
  it('should overwrite an existing file', async () => {
    const filepath = `${tmpDir}/name.json`;
    await current({ name: 'Geralt' }, filepath);
    await current({ name: 'Yennefer' }, filepath);
  ...
  it('should do nothing if given undefined as content', async () => {
    const filepath = `${tmpDir}/one/two/name.json`;
    await current(undefined, filepath);
  ...
  describe('sorting', () => {
    it('should sort the object keys by default', async () => {
      ...
      await current({ zoom: true, name: 'Geralt' }, filepath);
    ...
    it('should not sort the object keys with { sort: false }', async () => {
      const filepath = `${tmpDir}/file.json`;
      await current({ zoom: true, name: 'Geralt' }, filepath, { sort: false });
  ...
  describe('pretty print', () => {
    it('should pretty print the output', async () => {
      vi.spyOn(current, 'getPrettier').mockReturnValue(null);

      const filepath = `${tmpDir}/file.json`;
      await current({ name: 'Geralt', numbers: [1, 2, 3] }, filepath);
