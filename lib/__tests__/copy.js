...
describe('copy', () => {
  ...
  fit('with <placeholders>', async () => {
    // Given
    const userlandTmpDir = tmpDirectory('firost/copy');
    await mkdirp(absolute(userlandTmpDir, '.git'));
    await write('Something', absolute(userlandTmpDir, 'source.txt'));
    await write('Something else', absolute(userlandTmpDir, 'alternate.txt'));

    // When
    await runInUserland(
      dedent`
          const { copy } = await __import('./copy.js');
          await copy('<gitRoot>/source.txt', '<gitRoot>/lib/src/one.txt');
          await copy(
            ['<gitRoot>/source.txt', '<gitRoot>/alternate.txt'],
            '<gitRoot>/lib/array'
          );
          await copy('<gitRoot>/*.txt', '<gitRoot>/lib/glob');
        `,
    ...
    expect(actual).toEqual([
      'alternate.txt',
      'source.txt',
      'lib/array/alternate.txt',
      'lib/array/source.txt',
      'lib/glob/alternate.txt',
      'lib/glob/source.txt',
      'lib/src/one.txt',
    ]);
    // Cleanup
