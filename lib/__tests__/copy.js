  ...
  describe('with a simple path', () => {
    ...
    describe('symlinks', () => {
      ...
      beforeEach(async () => {
        ...
        await fs.ensureSymlink('./subdir/file.txt', linkFilePath);
      });
      it('should copy them by default', async () => {
        const outputPath = `${tmpDir}/dist/output.txt`;
        await current(linkFilePath, outputPath);

        const isSymlink = (await fs.lstat(outputPath)).isSymbolicLink();
        expect(isSymlink).toEqual(true);
      });
      it('should resolve them with resolveSymlinks: true', async () => {
        const outputPath = `${tmpDir}/dist/output.txt`;
        await current(linkFilePath, outputPath, { resolveSymlinks: true });

        const isSymlink = (await fs.lstat(outputPath)).isSymbolicLink();
        expect(isSymlink).toEqual(false);

        const actual = await read(outputPath);
        expect(actual).toEqual('real file');
      });
