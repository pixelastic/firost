...
describe('run', () => {
  ...
  it('should continue execution immediately when detached is true', async () => {
    const testDirectory = tmpDirectory('firost/run');
    await mkdirp(testDirectory);
    const testFile = `${testDirectory}/detached-marker.txt`;

    await captureOutput(async () => {
      await run(`sleep 1 && touch ${testFile}`, {
        shell: true,
        detached: true,
      });
    });
  ...
  describe('array form', () => {
    it.each([
      ...
      {
        title: 'preserves spaces in arguments',
        input: ['echo', 'foo bar'],
        expected: 'foo bar',
      },
      {
        title: 'treats shell metacharacters as literals',
        input: ['echo', 'a&b'],
        expected: 'a&b',
      },
      {
        title: 'passes literal quotes without interpretation',
        input: ['echo', '"hello"'],
        expected: '"hello"',
      },
  ...
  it('should allow parent Node.js process to exit when detached is true', async () => {
    const testDirectory = tmpDirectory('firost/run');
    await mkdirp(testDirectory);

    const runSourcePath = absolute(import.meta.dirname, '../run.js');

    // Create a test script that runs a long-running detached process
    const testScript = `${testDirectory}/test-detached-exit.js`;
    const scriptContent = `
import { run } from '${runSourcePath}';
await run('sleep 10', { detached: true });
`;

    await write(scriptContent, testScript);

    // Run the test script and measure how long it takes to exit
    const startTime = Date.now();
    await run(`node ${testScript}`);
    const elapsedTime = Date.now() - startTime;

    // The script should exit quickly, not wait 10s
    // Using 5s threshold to account for Node.js startup time under load
