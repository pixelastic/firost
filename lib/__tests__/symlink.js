...
describe('symlink', () => {
  ...
  describe('should understand <placeholders>', () => {
    it.each([
      [
        'in filepath only',
        {
          filepath: '<gitRoot>/tmp/symlink/link.txt',
          target: './file.txt',
        },
      ],
      [
        'in target only',
        {
          filepath: `${tmpDir}/link.txt`,
          target: '<gitRoot>/tmp/symlink/file.txt',
        },
      ],
      [
        'in both filepath and target',
        {
          filepath: '<gitRoot>/tmp/symlink/link.txt',
          target: '<gitRoot>/tmp/symlink/file.txt',
        },
      ...
      const actual = await read(`${tmpDir}/link.txt`);
