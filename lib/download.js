 ...
 **/
export async function download(targetUrl, userDestination) {
  let destination = absolute(userDestination, { cwd: callerDirectory() });
  ...
  try {
    ...
    const errorCode = `FIROST_DOWNLOAD_HTTP_${statusCode}`;
