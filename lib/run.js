 ...
 * error
 */
const module = async function run(command, userOptions = {}) {
  const options = {
    shell: false,
    stdout: true,
    stderr: true,
   ...
   * Note: I could not find a way to slience stdout/stderr while also allowing
   * keyboard input. Doing so was throwing errors. So wen stdin is set, I revert
   * to a simpler form
   **/
  async function withStdin() {
    execaOptions.stdio = 'inherit';
    return await module.__command(command, execaOptions);
  }
  ...
  async function withoutStdin() {
    const subprocess = module.__command(command, execaOptions);
...
module.__command = execa.command;
