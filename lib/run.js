 ...
 * error
 */
module.exports = async function run(command, userOptions = {}) {
  const options = {
    shell: false,
    stdout: true,
    stderr: true,
  ...
  async function withStdin() {
    ...
    return await this.__command(command, execaOptions);
  }
  ...
  async function withoutStdin() {
    const subprocess = this.__command(command, execaOptions);
...
module.exports.__command = execa.command;