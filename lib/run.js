...
/**
 * @param {string|string[]} command Command to run, as a string or array of
 * arguments
 ...
 * - detached (default: false) If set to true, run in background without waiting
 * @returns {object} Returns an object with .stdout and .sterr, or throws on
 * error
 */
export async function run(command, userOptions = {}) {
  const [commandBinary, ...commandArgs] = _.isArray(command)
    ? command
  ...
  // For detached processes, configure stdio to prevent parent from waiting
  if (options.detached) {
    execaOptions.stdio = 'ignore';
    execaOptions.cleanup = false;
  }
  ...
  async function withStdin() {
    ...
    return await __.execa(commandBinary, commandArgs, execaOptions);
  }
  ...
  async function withoutStdin() {
    const subprocess = __.execa(commandBinary, commandArgs, execaOptions);

    // Stop quickly in detached mode
    if (options.detached) {
      subprocess.unref();
    ...
    // Wait until the process is finished before returning
  ...
  try {
    ...
    const errorCode = `FIROST_RUN_EXIT_CODE_${err.exitCode}`;
    const newError = firostError(errorCode, err.message);
