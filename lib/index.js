...
const module = {
  ...
  urlToFilepath(input) {
    ...
    if (_.get(parsed, 'query')) {
      const extname = path.extname(_.get(parsed, 'pathname'));
      const basename = path.basename(_.get(parsed, 'pathname'), extname);
      ...
      filepath.push(path.basename(_.get(parsed, 'pathname')));
    }
